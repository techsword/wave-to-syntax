...
def seed_everything(seed):
    """Seed the RNGs used by the flat structural-probe path.

    ``seed=None`` (the default) performs no seeding, so the RNG stream and the
    results are unchanged from runs made without a seed. Pass an integer to
    make probe weight initialisation reproducible.
    """
    if seed is None:
        return
    random.seed(seed)
...
class TwoWordPSDProbe(Probe):
    ...
    def __init__(self, args):
        ...
        self.probe_rank = self.model_dim #args['probe']['maximum_rank']
...
class WordPairReporter(Reporter):
  ...
  def report_spearmanr(self, prediction_batches, dataset, split_name):
    ...
    for prediction_batch, (data_batch, label_batch, length_batch,_) in zip(
    ...
    with open(os.path.join(self.reporting_root, split_name + '.spearmanr-5_50-mean'), 'w') as fout:
      mean = np.nanmean([mean_spearman_for_each_length[x] for x in range(5,51) if x in mean_spearman_for_each_length])
      fout.write(str(mean) + '\n')
