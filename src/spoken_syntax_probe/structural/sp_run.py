"""Structural-probe runner.

Status: canonical (status by provenance: git dates tie, content superset plus
the ``wd``/``twd`` dispatch). This is the only structural-probe
runner; the legacy ``structuralprobe.py`` runner was removed on 2026-09-14.

The flat-schema probe, loss, and reporter classes live in
``spoken_syntax_probe.structural.flat_probe_utils``.
"""
...
from .regimen import ProbeRegimen
...
# Example extracted-embedding files produced by extract_segmented_embeddings.py
# during the original experiments. Your own files will differ; pass
# --embedding_file to use an arbitrary path, or index this list via the
# dataset_num positional argument. The file list is configurable, not a fixed
# requirement of the probe.
EMBEDDING_FILES = [
    'segmented_embeddings/wav2vec_small_spokencoco.pt',
    'segmented_embeddings/hubert-base-ls960_spokencoco.pt',
    'segmented_embeddings/fast-vgs_spokencoco.pt',
    'segmented_embeddings/fast-vgs-plus_spokencoco.pt',
    'segmented_embeddings/wav2vec2-large_spokencoco.pt',
    'segmented_embeddings/wav2vec2-large-english_spokencoco.pt',
    'segmented_embeddings/checkpoint-10000_spokencoco.pt',
    'segmented_embeddings/bert-base-uncased_spokencoco.pt',
    'segmented_embeddings/bert-large-uncased_spokencoco.pt',
    'segmented_embeddings/wav2vec2-pretrained-demo-50_segmented_spokencoco.pt',
    'segmented_embeddings/wav2vec2-pretrained-demo-10_segmented_spokencoco.pt',
    'segmented_embeddings/wav2vec2-pretrained-demo-30_segmented_spokencoco.pt',
    'segmented_embeddings/wav2vec2-pretrained-demo-20_segmented_spokencoco.pt',
    'segmented_embeddings/wav2vec2-demo-ft-mtl-30_segmented_spokencoco.pt',
    'segmented_embeddings/wav2vec2-demo-ft-mtl-50_segmented_spokencoco.pt',
    'segmented_embeddings/wav2vec2-base-repro-960h-libri-85k-steps_segmented_spokencoco.pt',
    'segmented_embeddings/wav2vec2-base_segmented_spokencoco.pt',
]
...
class LoadFromDisk_(Dataset):
    ...
    def __init__(self, data_path, transform=None):
        ...
        Args:
            ...
            transform (callable, optional): Optional transform to be applied
                on a sample.
        """
        self.emb, self.annot, self.audio_len = list(zip(*data_path))#torch.load(data_path)))
    ...
    def __getitem__(self, idx):
        ...
        #     embedding[lay] = self.emb[idx][lay]
...
def OneWordMain(args, emb_file, mode = 'wd'):
    seed_everything(args.get('seed'))
    scc_ds = torch.load(emb_file)
    datasetname = os.path.basename(emb_file).split('_')[-1][:-3]
    modelname = '_'.join(os.path.basename(emb_file).split('_')[:-1])
...
def interactive_run_this(result_dir='structural-probe-results/varying_mtl'):
    mode = 'twd'
    import os
    import pandas as pd
    import plotnine as p9
...
if __name__ == '__main__':
    ...
    # # parser.add_argument('modelname', nargs='?', default='wav2vec2_small', help='select the modelname')
    ...
    parser.add_argument('--embedding_file', default=None,
                        help='Path to a segmented embedding .pt file produced by '
                             'extract_segmented_embeddings.py. Overrides dataset_num.')
    parser.add_argument('--seed', type=int, default=None,
                        help='Seed random, numpy, and torch before probe training. '
                             'The private experiment results were produced without '
                             'seeding, so their seeds are unknown; leave unset to '
                             'keep the unseeded behaviour and RNG stream.')
    ...
    if cli_args.embedding_file is not None:
        emb_file = cli_args.embedding_file
    else:
        # EMBEDDING_FILES holds the example files used in the original experiments.
        # Your own files (from extract_segmented_embeddings.py) will differ; pass
        # --embedding_file to use an arbitrary file, or index this list by dataset_num.
        try:
            emb_file = EMBEDDING_FILES[int(cli_args.dataset_num)]
        except (ValueError, IndexError):
            parser.error(
                f'dataset_num must be an index in 0..{len(EMBEDDING_FILES)-1} '
                'or pass --embedding_file')
